...
**/
  ...
  user._users.post=function(data,callback){
      ...
      { //Check for all the required fields
        ...
        {
            callback(400,{'Error':' Required fieds are',name,email,password,address,pinCode});
        }     
      }
      else
      {
...
user._users.delete=function(data,callback){
               ...
               _data.delete('cart',email,function(err){
                 if(!err)
                 {
                   _data.delete('paymentHistory',email,function(err){
                     if(!err)
                     {callback(200);}
                     else{callback(400,{'Error':'Payment history does not exist'});}
                   })
                 }
                 else{
                   callback(400,{'Error':'Cart does not exist user deleted'});
                 }
               })
              }  
              else
    ...
    {
        callback(403,{"Error" : "email (query string) is missisng"});        
    }       
};
